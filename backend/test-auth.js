...
const testAuth = async () => {
  try {
    ...
    // Create a JWT token
    const token = jwt.sign(mockAdminUser, process.env.JWT_SECRET || 'your_jwt_secret_key_here', { expiresIn: '36500d' });
    ...
    const adminUsersResponse = await fetch('http://localhost:4095/api/v1/admin/users/users', {
      method: 'GET',
      headers: {
    ...
    const examSearchResponse = await fetch('http://localhost:4095/api/v1/exam-results/search?page=1&limit=20', {
      method: 'GET',
      headers: {
        'Content-Type': 'application/json',
        'Cookie': `token=${token}`
