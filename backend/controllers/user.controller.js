...
const login = async (req, res, next) => {
    try {
        ...
        res.cookie('token', tokens.accessToken, {
            ...cookieOptions,
            maxAge: 3650 * 24 * 60 * 60 * 1000, // 3650 days (~10 years) - match JWT expiration
            expires: new Date(Date.now() + 3650 * 24 * 60 * 60 * 1000)
        });

        // Set refresh token as httpOnly cookie (very long-lived ~10 years)
        res.cookie('refreshToken', tokens.refreshToken, {
            ...cookieOptions,
            maxAge: 3650 * 24 * 60 * 60 * 1000, // 3650 days (~10 years)
            expires: new Date(Date.now() + 3650 * 24 * 60 * 60 * 1000)
        });
...
const refreshToken = async (req, res, next) => {
    try {
        ...
        const { refreshToken: tokenFromCookies } = req.cookies;
        ...
        const decoded = jwt.verify(tokenFromCookies, process.env.JWT_REFRESH_SECRET || process.env.JWT_SECRET);
        ...
        // Set new access token as httpOnly cookie (long-lived to match JWT expiration)
        res.cookie('token', tokens.accessToken, {
            ...cookieOptions,
            maxAge: 3650 * 24 * 60 * 60 * 1000, // 3650 days (~10 years) - match JWT expiration
            expires: new Date(Date.now() + 3650 * 24 * 60 * 60 * 1000)
        ...
        // Set new refresh token as httpOnly cookie (very long-lived ~10 years)
        res.cookie('refreshToken', tokens.refreshToken, {
            ...cookieOptions,
            maxAge: 3650 * 24 * 60 * 60 * 1000, // 3650 days (~10 years)
            expires: new Date(Date.now() + 3650 * 24 * 60 * 60 * 1000)
