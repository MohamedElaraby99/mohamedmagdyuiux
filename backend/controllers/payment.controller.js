...
export const purchaseCourse = asyncHandler(async (req, res) => {
    ...
    if (price <= 0) {
        throw new ApiError(400, "This course is free and doesn't require purchase");
    }
    ...
    user.wallet.transactions.push({
        type: 'purchase',
        amount: price,
        code: `COURSE_${purchase._id.toString().slice(-8).toUpperCase()}`,
        description: `شراء كورس: ${course.title}`,
        date: new Date(),
        status: 'completed'
    });
...
export const checkCoursePurchaseStatus = asyncHandler(async (req, res) => {
    ...
    return res.status(200).json(
        new ApiResponse(200, {
            isPurchased: !!purchase,
            purchase
        }, "Course purchase status checked successfully")
    );
});
