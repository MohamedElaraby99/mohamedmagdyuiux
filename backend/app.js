...
const allowedOrigins = new Set([
  ...
  'http://localhost:5190',
  'http://127.0.0.1:5190',
  'https://magdyacademy.com',
  'https://www.magdyacademy.com',
  'https://api.magdyacademy.com'
]);
...
// Configure CORS using the standard 'cors' package
app.use(cors({
  origin: (origin, callback) => {
    ...
    // For now, in development, we might want to be more permissive if needed
    // But sticking to the allowlist is safer. 
  ...
  allowedHeaders: ['Content-Type', 'Authorization', 'x-auth-token', 'x-device-info'],
...
app.get('/api/v1/test-uploads', (req, res) => {
  ...
  try {
    ...
    const baseUrl = process.env.BACKEND_URL || `http://localhost:${process.env.PORT || 4095}`;
