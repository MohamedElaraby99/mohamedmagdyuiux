...
export const listCourseAccessCodes = asyncHandler(async (req, res) => {
    const { courseId, isUsed, createdFrom, createdTo } = req.query;
    ...
    // Add date filtering for createdAt
    ...
    // For non-search queries, apply date filtering to the regular find query
    if (createdFrom || createdTo) {
        // This handles the case where we have date filters but no search query
        // The regular find query already includes the date filtering we added above
    }
