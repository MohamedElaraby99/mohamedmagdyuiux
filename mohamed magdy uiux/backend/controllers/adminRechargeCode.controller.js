import rechargeComohamedmagdyuiuxdel from "../models/rechargeCode.model.js";
...
const generateRechargeCode = async (req, res, next) => {
    try {
        ...
        for (let i = 0; i < quantity; i++) {
            ...
            while (!isUnique) {
                code = rechargeComohamedmagdyuiuxdel.generateCode();
                const existingCode = await rechargeComohamedmagdyuiuxdel.findOne({ code });
                if (!existingCode) {
                    isUnique = true;
                }
            }

            const rechargeCode = await rechargeComohamedmagdyuiuxdel.create({
                code,
...
const getAllRechargeCodes = async (req, res, next) => {
    try {
        ...
        const codes = await rechargeComohamedmagdyuiuxdel.find(query)
        ...
        const total = await rechargeComohamedmagdyuiuxdel.countDocuments(query);
        const totalPages = Math.ceil(total / limit);

        // Calculate statistics
        const stats = await rechargeComohamedmagdyuiuxdel.aggregate([
            {
                $group: {
                    _id: null,
                    totalCodes: { $sum: 1 },
...
const deleteRechargeCode = async (req, res, next) => {
    try {
        ...
        const code = await rechargeComohamedmagdyuiuxdel.findById(id);
        if (!code) {
        ...
        await rechargeComohamedmagdyuiuxdel.findByIdAndDelete(id);
...
const getRechargeCodeStats = async (req, res, next) => {
    try {
        ...
        // Get recent codes
        const recentCodes = await rechargeComohamedmagdyuiuxdel.find()
            .sort({ createdAt: -1 })
            .limit(5)
            .populate('createdBy', 'fullName');

        // Get monthly stats
        const monthlyStats = await rechargeComohamedmagdyuiuxdel.aggregate([
            {
                $group: {
                    _id: {
                        year: { $year: '$createdAt' },
