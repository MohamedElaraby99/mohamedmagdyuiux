...
export const getAdminCourses = async (req, res, next) => {
  try {
    ...
    const courses = await Course.find(query)
      ...
      .populate({
        path: 'units.lessons.trainings.userAttempts.userId',
        select: 'fullName email phoneNumber'
      })
      .populate({
        path: 'units.lessons.entryExam.userAttempts.userId',
        select: 'fullName email phoneNumber'
      })
      .populate({
        path: 'directLessons.exams.userAttempts.userId',
        select: 'fullName email phoneNumber'
      })
      .populate({
        path: 'directLessons.trainings.userAttempts.userId',
        select: 'fullName email phoneNumber'
      })
      .populate({
        path: 'directLessons.entryExam.userAttempts.userId',
        select: 'fullName email phoneNumber'
      });
...
// Get course by ID (full content for admin, secure version for public viewing)
export const getCourseById = async (req, res, next) => {
  try {
    ...
    if (isAdmin) {
      // Admin users get full course data with all content

      return res.status(200).json({ success: true, data: { course } });
    }

    // Regular users get secure version without sensitive data
...
export const getLessonById = async (req, res, next) => {
  try {
    ...
    let contentUnlocked = true; // Default to unlocked
    ...
    const optimizedLesson = {
      ...
      entryExam: processedEntryExam,
      // Only show content if unlocked OR if entry exam not required
      videos: contentUnlocked ? filteredVideos.map(video => ({
        _id: video._id,
        url: video.url,
        title: video.title,
      ...
      // Lock exams and trainings too!
      exams: contentUnlocked ? processedExams : [],
      trainings: contentUnlocked ? processedTrainings : [],
      // Show count of locked content if not unlocked
      lockedVideosCount: contentUnlocked ? 0 : filteredVideos.length,
      lockedPdfsCount: contentUnlocked ? 0 : filteredPdfs.length,
      lockedExamsCount: contentUnlocked ? 0 : processedExams.length,
      lockedTrainingsCount: contentUnlocked ? 0 : processedTrainings.length
    };
