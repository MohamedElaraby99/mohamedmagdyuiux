...
// Top active users by number of exam attempts
const getTopActiveUsers = asyncHandler(async (req, res) => {
    try {
        ...
        const match = {};
        if (courseId) {
            try { match.course = new mongoose.Types.ObjectId(courseId); } catch { match.course = courseId; }
        }
        ...
        const pipeline = [
            ...
            { $match: { attempts: { $gte: parseInt(minAttempts) } } },
            { $sort: { attempts: -1, lastAttemptAt: -1 } },
            { $limit: parseInt(limit) },
            {
                $lookup: {
                    from: 'users',
        ...
        // Optional stage filter (by user's stage)
        if (stageId) {
            pipeline.push({ $match: { 'user.stage': (function() { try { return new mongoose.Types.ObjectId(stageId); } catch { return stageId; } })() } });
        }
        ...
        pipeline.push({
            $project: {
                ...
                attempts: 1,
                lastAttemptAt: 1,
                averageScore: { $ifNull: [{ $round: ['$averageScore', 0] }, 0] },
                user: {
                    _id: '$user._id',
        ...
        throw new AppError('فشل في الحصول على أكثر الطلاب نشاطاً', 500);
    }
});
