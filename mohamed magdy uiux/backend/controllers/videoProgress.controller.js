...
const getAllUsersProgressSummary = async (req, res, next) => {
  try {
    ...
    // Get all unique course IDs from the user progress data
    const allCourseIds = [...new Set(usersProgress.flatMap(u =>
      u.recentVideos.map(v => v.courseId?.toString()).filter(Boolean)
    ))];

    // Fetch courses with their full structure for video title lookup
    const coursesWithStructure = await courseModel.find({
      _id: { $in: allCourseIds }
    }).select('title units directLessons');

    // Create a map for quick course lookup
    const courseMap = new Map();
    coursesWithStructure.forEach(course => {
      courseMap.set(course._id.toString(), course);
    });

    // Format the data with video titles
    const formattedData = usersProgress.map(item => ({
      ...
      recentActivity: item.recentVideos
        ...
        .slice(0, 5)
        .map(video => {
          const course = courseMap.get(video.courseId?.toString());
          const videoTitle = findVideoTitle(course, video.videoId);
          return {
            ...video,
            videoTitle: videoTitle || video.courseName || `فيديو: ${video.videoId}`
          };
        })
    }));
