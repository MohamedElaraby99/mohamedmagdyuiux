...
export const purchaseCourse = createAsyncThunk(
  'payment/purchaseCourse',
  async ({ courseId }, { rejectWithValue }) => {
    try {
      const response = await axiosInstance.post('/payments/purchase-course', {
        courseId
      });
      return response.data;
    } catch (error) {
      return rejectWithValue(error.response?.data || 'Course purchase failed');
    }
  }
);

export const checkCoursePurchaseStatus = createAsyncThunk(
  'payment/checkCoursePurchaseStatus',
  async ({ courseId }, { rejectWithValue }) => {
    try {
      const response = await axiosInstance.get('/payments/check-course-purchase', {
        params: { courseId }
      });
      return response.data;
    } catch (error) {
      return rejectWithValue(error.response?.data || 'Failed to check course purchase status');
    }
  }
);
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(purchaseCourse.fulfilled, (state, action) => {
        ...
        const { courseId } = action.payload.data.purchase;
        state.coursePurchaseStatus[courseId] = true;
      })
      .addCase(purchaseCourse.rejected, (state, action) => {
        state.loading = false;
        state.error = action.payload?.message || 'Course purchase failed';
      })

      // Check Course Purchase Status
      .addCase(checkCoursePurchaseStatus.fulfilled, (state, action) => {
        const { courseId } = action.meta.arg;
        state.coursePurchaseStatus[courseId] = action.payload.data.isPurchased;
      });
  }
});
