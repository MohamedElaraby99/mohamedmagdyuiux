...
const CustomVideoPlayer = ({
  ...
  // Check if device is mobile (phones only, not tablets) and handle orientation
  useEffect(() => {
    const checkDeviceType = () => {
      // Detect mobile phones only (exclude tablets like iPad)
      const isMobileDevice = /Android|webOS|iPhone|iPod|BlackBerry|IEMobile|Opera Mini/i.test(navigator.userAgent) &&
                           !/iPad/i.test(navigator.userAgent);
      setIsMobile(isMobileDevice);

      if (isMobileDevice) {
        // Request fullscreen and landscape on mobile phones
          ...
          {isLoading && (
            <div className="absolute inset-0 flex items-center justify-center bg-black bg-opacity-75 z-10">
              <div className="text-center">
                ...
                <p className="text-white text-sm mt-2">جاري تهيئة المشغل...</p>
                {isMobile && (
                  <p className="text-white text-sm mt-2">اقلب الجهاز للمشاهدة الأفضل</p>
                )}
              </div>
            </div>
          )}

          {/* Mobile Rotation Indicator - Simple Arabic version for phones only */}
          {isMobile && !isLandscape && !isLoading && (
            <div className="absolute inset-0 flex items-center justify-center bg-black bg-opacity-90 z-20">
              <div className="text-center p-6">
                <p className="text-white text-xl font-bold mb-2">اقلب الجهاز</p>
                <p className="text-gray-300 text-base mb-4">للحصول على أفضل تجربة مشاهدة</p>
                <button
                  onClick={requestFullscreenAndLandscape}
                  className="px-6 py-3 bg-green-500 text-white rounded-lg text-lg font-medium hover:bg-green-600 transition-colors"
                >
                  تشغيل تلقائي
                </button>
              </div>
              ...
              <div className="absolute bottom-0 left-0 right-0 p-4 pointer-events-auto">
                ...
                <div className="mb-4">
                  ...
                  >
                    ...
                    {/* Progress indicator */}
                    <div 
                      className="absolute top-1/2 transform -translate-y-1/2 w-3 h-3 bg-green-500 rounded-full shadow-lg z-20"
