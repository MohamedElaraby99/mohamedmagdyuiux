...
export default function Attendance() {
  ...
  const getStudentAttendanceStatus = (studentId) => {
    const record = attendanceRecords.find(record => {
      const recordStudentId = record.student?._id || record.student?.id || record.userId;
      return recordStudentId === studentId && record.date === selectedDate;
    });
                  ...
                  <table className="w-full text-sm">
                    ...
                    <tbody className="divide-y divide-gray-200 dark:divide-gray-600">
                      {filteredStudents.map((student) => {
                        ...
                        const attendanceStatus = getStudentAttendanceStatus(studentId);
                        return (
                          <tr key={studentId} className="hover:bg-gray-50 dark:hover:bg-gray-700">
                            ...
                            <td className="px-4 py-3">
                              <div className="flex items-center space-x-2 space-x-reverse">
                                ...
                                <button
                                  onClick={() => handleManualAttendance(studentId, 'late')}
                                ...
                                <button
                                  onClick={() => handleManualAttendance(studentId, 'absent')}
                ...
                <tbody className="divide-y divide-gray-200 dark:divide-gray-600">
                  {allAttendance.data && allAttendance.data.length > 0 ? (
                    allAttendance.data.map((record) => {
                      // Look up group name - try record.group first, then look up by groupId
                      const recordId = record._id || record.id;
                      const groupId = record.group?._id || record.group?.id || record.groupId || record.group;
                      const foundGroup = groups.find(g => (g._id || g.id) === groupId);
                      const groupName = record.group?.name || foundGroup?.name || 'غير محدد';
