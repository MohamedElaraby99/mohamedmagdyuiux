...
export const createStage = createAsyncThunk(
  ...
  async (stageData, { rejectWithValue }) => {
    try {
      // Check if stageData is FormData (for file uploads)
      const isFormData = stageData instanceof FormData;
      const config = isFormData ? {
        headers: { 'Content-Type': 'multipart/form-data' }
      } : {};

      const response = await axiosInstance.post('/stages', stageData, config);
...
export const updateStage = createAsyncThunk(
  ...
  async ({ id, stageData }, { rejectWithValue }) => {
    try {
      ...
      const response = await axiosInstance.put(`/stages/${id}`, stageData, config);
